...
3. **Contiene il vettore nullo**: Il sistema omogeneo ha sempre la soluzione $\mathbf{x} = \mathbf{0}$, poiché $A0 = 0$.
...
Sia $V$ uno spazio vettoriale, la **combinazione lineare** di $k$ vettori $v_{1},...,v_{k}\in V$ con coefficienti $\alpha_{1},...,\alpha_{k}\in \mathbb{R}$ è il vettore $\alpha_{1}v_{1}+...+a_{k}v_{k}\in V.$
...
Dimostrazione:
	Per dimostrare che è chiuso rispetto alla somma dobbiamo far vedere che la somma di du combinazioni lineari di $v_{1},...,v_{k}$ è ancora una combinazione lineare di $v_{1},...,v_{k}$. 
...
In questo caso, lo $Span(v_{1},v_{2})$ è il piano $z=0,$ ovvero tutti i vettori della forma $\begin{vmatrix} a_{1}\\a_{2}\\0.\end{vmatrix}.$ Infatti è facile verificare che per ogni $a_{1}, a_{2}\in \mathbb{R}$ esistono $\alpha_{1}, \alpha_{2}\in \mathbb{R}$ tali che$$\begin{cases} 3\alpha_{1}-\alpha_{2}=a_{1} \\ \alpha_{1}-\alpha_{2}=a_{2}\end{cases}\implies\begin{cases} 3\alpha_{2}+3a_{2}-\alpha_{2}=a_{1}\\\alpha_{1}=\alpha_{2}+a_{2}\end{cases}\implies\begin{cases}\alpha_{2}=\frac{a_{1}-3a_{2}}{2}\\\alpha_{1}=\frac{a_{1}-a_{2}}{2}\end{cases}$$per cui $$\begin{vmatrix} b_1\\b_{2}\\0\end{vmatrix}=\alpha_{1}v_{1}+\alpha_{2}v_{2}\in Span(v_{1},v_{2}).$$
...
- Lo Span di k-vettori è un sottospazio vettoriale. 
- Un sistema è risolubile se e solo se la colonna dei termini noti appartiene allo Span delle colonne della matrice dei coefficienti del sistema.
